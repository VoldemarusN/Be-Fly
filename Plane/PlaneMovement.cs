...
namespace Plane
{
    public class PlaneMovement : IPlaneMovement
    {
        ...
        /*public void ExecuteWindPhysic()
        {
            if (_inputService.HoldOnScreen.IsPressed())
            {
                ApplyTorque(_planeConfig.PlanePhysics.AccelerationUp);
                AddForce(_planeConfig.PlanePhysics.AccelerationRight);
            }
            else
                ApplyGravity();

            AddForce(_windConfig.WindForce);
        }*/
    }
}