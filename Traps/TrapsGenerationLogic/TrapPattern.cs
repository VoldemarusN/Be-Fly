...
namespace Traps.TrapsGenerationLogic
{
    public class TrapPattern : MonoBehaviour
    {
        ...
        public void InitializeTraps()
        {
            ...
            {
                baseTrap.OnDestroyed += trap => OnTrapDestroyed?.Invoke(trap);
            }
        ...
        private void HandleCreatedTraps()
        {
            foreach (var baseTrap in Traps)
            {
                ...
                if (_disabledTrapsCount == Traps.Count()) OnAllTrapsDeactivated?.Invoke(this);
            }
        }
        ...
        private void GenerateChunks(GameObject originChunk)
        {
            ...
            {
                Vector3 position = new Vector3(transform.position.x, transform.position.y + i * _spaceBetweenChunks, 0f);
