...
namespace UI.View
{
    public class UpgradeViewManager : MonoBehaviour
    {
        ...
        public void SetCustomTextToUpgradeType(string dataCharacteristic, string complete)
        {
            foreach (var view in _instantiatedUpgradeViews)
            {
                if (view.CharacteristicName == dataCharacteristic) view.SetCustomText(complete);
            }
        }
