...
namespace UI.Garage
{
    public class PassiveUpgradeController : IInitializable
    {
        ...
        private GaragePlaneView GetPlaneView(PlaneType type) =>
            _planeScroller.GaragePlaneViews.First(view => view.PlaneType == type);

        private void InitializePlaneSprites()
        {
            foreach (var planeView in _planeScroller.GaragePlaneViews)
            {
                var type = planeView.PlaneType;
                var planeData = _storage.GetPlaneData(type);
                planeView.SetLevel(planeData.Level);

                if (planeData.IsUnlocked == false) planeView.ProgressBar.Hide();
                else planeView.ProgressBar.SetProgressNormalized(planeData.LevelNormalized);
            }
        }
        ...
        private void OnPlaneSwitched(PlaneType planeType)
        {
            ...
            if (planeData.Level != resistanceLevel && planeType != PlaneType.Paper) PlayUpgradeEffects();
